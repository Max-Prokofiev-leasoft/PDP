...
class PdpSkillController extends Controller
{
    ...
    public function recentClosures(Request $request)
    {
        // Show items approved in the last N days (default 5). Optional filter by specific PDP.
        $days = max(1, (int)$request->query('days', 5));
        $pdpId = $request->query('pdp');
        $uid = $request->user()->id;

        $tz = new \DateTimeZone(config('app.timezone', 'UTC'));
        $to = new \DateTime('now', $tz);
        $from = (clone $to)->modify('-' . ($days - 1) . ' days'); // inclusive window across calendar days
        $from->setTime(0,0,0);

        $q = \App\Models\PdpSkillCriterionProgress::query()
            ->join('pdp_skills', 'pdp_skills.id', '=', 'pdp_skill_criterion_progress.pdp_skill_id')
            ->join('pdps', 'pdps.id', '=', 'pdp_skills.pdp_id')
            ->where('pdp_skill_criterion_progress.approved', true)
            ->whereBetween('pdp_skill_criterion_progress.updated_at', [
        ...
        if ($pdpId) {
            ...
            if ($pdp) {
                $this->authorizePdp($request, $pdp);
                $q->where('pdps.id', $pdp->id);
            } else {
                return response()->json([]);
            }
        } else {
            // Scope to PDPS current user owns or curates
            $q->where(function($w) use ($uid) {
                $w->where('pdps.user_id', $uid)
        ...
        $rows = $q->orderByDesc('pdp_skill_criterion_progress.updated_at')
            ->limit(20)
            ->get([
                'pdp_skill_criterion_progress.id as id',
                'pdp_skill_criterion_progress.pdp_skill_id as skill_id',
                ...
                'pdp_skill_criterion_progress.user_id as author_id',
                'pdp_skill_criterion_progress.note as note',
                'pdp_skill_criterion_progress.updated_at as closed_at',
                'pdp_skills.pdp_id as pdp_id',
                'pdp_skills.skill as skill',
                'pdp_skills.criteria as criteria',
                'pdps.title as pdp_title',
        ...
        foreach ($rows as $row) {
            ...
            $out[] = [
                'id' => (int)$row->id,
                'pdp' => ['id' => (int)$row->pdp_id, 'title' => (string)$row->pdp_title],
                'skill' => ['id' => (int)$row->skill_id, 'name' => (string)$row->skill],
                'criterion' => ['index' => $idx, 'text' => $criterionText],
                'note' => (string)$row->note,
